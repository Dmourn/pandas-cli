...
class BasePanda(object):
    ...
    def make_mi(self):
        selected = BaseUtils.show_choices(self.get_wk_cols(), msg='make_mi')
        for i in selected:
    ...
    def sort(self):
        selected = BaseUtils.show_choices(self.working_frame.columns.to_list(), msg='Select column to sort by', once=True)
    ...
    def sort_mi(self):
        selected = BaseUtils.show_choices(list(self.MI.names), msg='Select item to sort MI', once=True)
        print(f'len(selected)={len(selected)}')
        try:
    ...
    def swap_mi(self):
        selected = BaseUtils.show_choices(self.MI.names)
        if len(selected) == 2:
            try:
    ...
    def search(self):
        ...
        selected=BaseUtils.show_choices(self.get_wk_cols(),msg='column to search on',once=True)
    ...
    def drop_rows(self):
        selected=BaseUtils.show_choices(self.get_wk_rows(), msg='rows to drop', once=False)
    ...
    def drop_cols(self):
        selected=BaseUtils.show_choices(self.get_wk_cols(),msg='columns to drop',once=False)
        self.working_frame.drop(selected, axis=1, inplace=True)

    # string to list
    def split(self, pat='\n\n'):
        print("creates a new frame/series what now")
        selected=BaseUtils.show_choices(self.get_wk_cols(), msg='column to split',once=True)
        B=self.working_frame[selected].str.split(pat=pat)
        return B
    
    #list to b.a.p. (big ass panda)
    def explode(self):
        selected=BaseUtils.show_choices(self.get_wk_cols(),msg='column to "Explode"',once=True)
        x=input(f'{selected}: ')
        #B=self.working_frame.copy()
        
    #takes a row and makes it the column header
    def pop_cols(self):
        selected=BaseUtils.show_choices(self.get_wk_rows(), msg='row to replace columns', once=True)
    ...
    def show_a_column(self):
        ...
        selected=BaseUtils.show_choices(self.get_wk_cols(), msg='Select a column',)
...
class JSONPanda(BasePanda):
    ...
    a data field, I'll throw a load_file method on the BasePanda
    """
    def get_data(self):
